...
function Documark( basePath ) {
	...
	this.config  = null;
	this.content = null;
	this.plugins = new PluginProvider();
	this.debug   = 0;
}
...
Documark.prototype.load = function() {
	...
	// Reload plugins
	this.plugins.set( this.getConfig().plugins );
};
...
Documark.prototype.getParser = function() {
	...
	try {
		...
		throw new Error( 'Error loading .' + ext + ' parser:\n' + e );
	}
};
...
Documark.prototype._createPDFGenerator = function( config ) {
	var basePath = this.getPath();
	var options  = [];
	var pdf;
	...
	this.emit( 'plugin:pdf', options );

	pdf = spawn( 'wkhtmltopdf', options, { cwd: basePath, stdio: 'pipe' } );
