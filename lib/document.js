...
function Document (documentFile) {
	...
	// Make Document an event emitter
	...
	this.applyPlugins = function ($, plugins, done) {
		var plugin;
	...
	this.handleDocumentPlugins = function ($) {
		...
		// Keep global list of plugins,
		// so plugins themselves can add to the list
		plugins = self.config().plugins;

		if (Array.isArray(plugins)) {
			// Copy and dereference, to leave original untouched
			plugins = plugins.slice(0);
		} else {
			plugins = [];
		}

		self.applyPlugins($, plugins, function (err) {
	...
	this.compileToPDF = function ($) {
		...
		var file = cacheHelper(self).fileWriteStream('dump.html');
		...
		pdf.convert({
			...
			result.toFile(self.outputFile(), function () {
				deferred.resolve();
			});
		});
