...
function Documark (basePath) {
	...
	if (fs.statSync(basePath).isFile()) {
		file = basePath;
	} else if (fs.fileExistsSync(path.join(basePath, 'document.jade'))) {
		file = path.join(basePath, 'document.jade');
	} else {
		throw new Error('No document.jade file found in ' + basePath);
	}
	...
	this.file = function () {
		if (file === null) {
			...
			var files = glob.sync(path.join(this.path(), 'document.jade'));

			if ( ! files.length) {
				throw new Error('No document.jade file found in ' + this.path());
			}

			// TODO: Test if files in subdirectory do not come first. We want ./document.jade.
			file = files[0];
		}
	...
	this.parser = function () {
		var type = path.extname(this.file()).replace(/[^a-z0-9\-]/gi, '');

		return new Parser(type, this);
	};
